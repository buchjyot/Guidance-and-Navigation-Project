...
%   in radians at that location.  Inputs are in degrees (lat and lon) and
...
%   Created 4/7/00 
%   Last Modified 06/02/06 (Yunfeng Shao)
%---------------------------------------------------------------------

ecef = wgslla2xyz(lat,lon,h);
ecef = ecef/1000;
...
Hn = wgsxyz2ned2(Hn_ecef,1000*ecef);
dipA = atan(Hn(3)/sqrt(Hn(1)^2+Hn(2)^2));
decA = atan(Hn(2)/Hn(1));
